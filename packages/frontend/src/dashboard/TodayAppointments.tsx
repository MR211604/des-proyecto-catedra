    ...
    <section className="overflow-hidden rounded-[9px] border border-[#e8dce6] bg-[#fff7fc] shadow-[0_5px_16px_rgb(75_49_69/6%)]">
      ...
      <div className="grid gap-4 px-5 pt-5 pb-1.5">
        {appointments.map(([time, period, title, detail, tone]) => (
          <article
            className={`flex min-h-30.25 items-center gap-5 rounded-r border-l-4 bg-white px-5 py-3.75 shadow-[0_1px_5px_rgb(70_45_63/5%)] max-[480px]:gap-2.5 max-[480px]:px-3 ${tone}`}
            key={title}
          >
            <div className="w-17.5 flex-[0_0_70px] border-r border-[#eee3eb] pr-4.5 text-center max-[480px]:w-15 max-[480px]:basis-15 max-[480px]:pr-2">
              <strong className="block text-[17px]">{time}</strong>
              <span className="mt-0.75 block text-[13px]">{period}</span>
            </div>
            <div>
              <strong className="block text-[17px] max-[480px]:text-[15px]">
                {title}
              </strong>
              <span className="mt-1.75 block leading-5.5 text-[#4e444b]">
                {detail}
              </span>
            </div>
          </article>
      ...
      <button
        className="mx-5 mb-5 mt-6.5 h-12 w-[calc(100%-40px)] cursor-pointer border border-[#8b5e83] bg-transparent font-bold text-[#8b5e83]"
        type="button"
      >
        Ver Calendario
      </button>
